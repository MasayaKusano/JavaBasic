	...
	public static void main(String[] args) {
		...
		for (int count = 0; count < users.length; count++) {
			...
			// ★ 変数lineに格納されている値を','で区切り、配列にしてください（Stringクラスのメソッドを使用します）
			 String[] aaaa = line.split(",");

			// ★ 配列にした値を、users[カウント]の各フィールドに代入してください
			// ★ ユーザIDに該当する値が数値でなかった場合は、エラーでプログラムが止まっても良いです
			 users[count] = new User();
			users[count].userId = Integer.parseInt(aaaa[0]);
			users[count].userNm =aaaa[1];
			users[count].mail =aaaa[2];
			users[count].password =aaaa[3];
		...
		for (int count = 0; count < users.length; count++) {
			System.out.println(users[count].userId);
			System.out.println(users[count].userNm);
			System.out.println(users[count].mail);
			System.out.println(users[count].password);
		}
	}
	}
